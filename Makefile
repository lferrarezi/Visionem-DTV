...
$(BUILD_DIR)/siano-tv: src/tuner-cli/siano_tv.c src/libsmsusb/smsusb_transport.c src/libsmsusb/smsusb_transport.h | $(BUILD_DIR)
	$(CC) $(CFLAGS) $(LIBUSB_CFLAGS) -Isrc/libsmsusb src/tuner-cli/siano_tv.c src/libsmsusb/smsusb_transport.c -o $@ $(LIBUSB_LDFLAGS)
...
install: all
	...
	if [ -f firmware/dvb_nova_12mhz_b0.inp ]; then install -m 0644 firmware/dvb_nova_12mhz_b0.inp "$(DATADIR)/firmware/dvb_nova_12mhz_b0.inp"; fi
